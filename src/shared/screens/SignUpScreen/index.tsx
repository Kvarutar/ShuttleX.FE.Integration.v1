...
import { type SignUpForm, type SignUpFormValidation, type SignUpProps } from './types';

const SignUpScreenWithoutI18n = ({
  navigateToSignIn,
  navigateToTerms,
  navigateToSingUpCode,
  ...
  const validateField = useCallback((field: keyof SignUpForm, value: string | boolean): boolean => {
    switch (field) {
      case 'firstName':
  ...
  useEffect(() => {
    const isInitiallyFormValid = Object.values(userDataForm).every(
      value => value !== '' && value !== null && value !== false,
    );
    setIsButtonEnabled(wasValidated ? validateForm() : isInitiallyFormValid);
  }, [userDataForm, validateForm, wasValidated]);
  ...
  return (
    <>
      <ScrollViewWithCustomScroll
        contentContainerStyle={[styles.formSignUpContainer, computedStyles.formSignUpContainer]}
      >
      ...
      <View style={styles.buttonsContainer}>
        ...
        <Pressable style={styles.alreadyHaveAccountContainer} onPress={navigateToSignIn} hitSlop={20}>
          <Text style={styles.alreadyHaveAccountText}>
            ...
            <Text style={[styles.signInLabel, computedStyles.signInLabel]}>{t('SignUp_signInButton')}</Text>
          </Text>
        </Pressable>
      </View>
